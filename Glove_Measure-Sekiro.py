    ...
    def getContours(img, imgContour):
        ...
        for cnt in contours:
            ...
            if area> 1000:
                ...
                # cv2.putText(imgContour, "1", (x_ + w + 0, y_ + 0), cv2.FONT_HERSHEY_COMPLEX, .7,
                #             (0, 255, 0), 2)
                p1 = (x_ + w , y_ )
                # cv2.putText(imgContour, "2", (x_ + 0, y_ + h + 0), cv2.FONT_HERSHEY_COMPLEX, .7,
                #            (0, 255, 0), 2)
                p2 = (x_ , y_ + h )
                ...
                # for x in tipPoint:
                #     print (x)
                gapLength =(distanceCalculate(p3, p2))/10
                format_gapLength = "{:.2f}".format(gapLength)
                # ddd = math.sqrt((x2 - x1)**2 + (y2 - y1)**2)
                #print (gapLength)
